...
`review-pr` runs a configurable `N → N → 1` pull-request review pipeline in dedicated Git checkouts, with an optional standalone comparison report after the core final synthesis. It supports Claude Code, Codex, Pi, and custom command-line adapters. This package installs the same orchestrator on Linux and macOS; it does not bundle or purchase agent CLIs or models. This release is version `1.21.0`.
...
git checkout v1.21.0        # or stay on main for the development version
```
...
```text
dist/review-pr-1.21.0.tar.gz
dist/review-pr-1.21.0.tar.gz.sha256
```
...
```bash
gh release download v1.21.0 --repo oforostianyi/review-pr --pattern 'review-pr-*'
```
...
```bash
sha256sum -c review-pr-1.21.0.tar.gz.sha256   # Linux
# or: shasum -a 256 -c review-pr-1.21.0.tar.gz.sha256  # macOS
tar -xzf review-pr-1.21.0.tar.gz
cd review-pr-1.21.0
./install.sh --repo "$HOME/Review" --reviews-dir "$HOME/review-pr"
```
...
- **Invalid structured output:** the failure reason names the field, for example
  ...
  When the repair or continuation cannot rescue the stream either, the phase is not abandoned: every
  record is put to the contract on its own, the ones that pass are published, and the ones that do
  not are dropped and named. The console says what was lost and why, the manifest gains a
  `salvage_losses` entry, and the review itself lists the loss among its verification limitations. A
  phase fails only when no record survives. The reasoning is that by then every agent has already
  run: refusing to publish would throw away the surviving work along with the broken part, and leave
  nothing but a log to explain hours of spent quota.
