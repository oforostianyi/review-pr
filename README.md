...
`reporting.finding_contract.primary` accepts `markdown` (the backward-compatible default) or opt-in `ndjson-v1`. In structured mode, primary agents return atomic NDJSON records plus a terminal completion record. The orchestrator validates them against the exact changed-line map, preserves `work/*-raw.ndjson`, publishes canonical `work/*-findings.json`, and renders localized Markdown for cross-review. A structurally repairable response receives one dedicated schema-repair pass before an ordinary whole-review retry: every original finding must already be parseable and complete, and the repaired stream may not add, remove, reorder, translate, or change any substantive finding field. The original response, rejected repair output, diagnostics, and per-pass usage are preserved as applicable. Unsafe or still-invalid repair fails closed and never falls back to Markdown. Existing manifests without the setting remain Markdown runs; resume uses the mode saved in the manifest. The installed `review-pr-findings-v1.schema.json` documents the portable record shape. Cross-review and final synthesis remain Markdown in this staged rollout.
