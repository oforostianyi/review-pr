...
`review-pr` runs a configurable `N → N → 1` pull-request review pipeline in dedicated Git checkouts, with an optional standalone comparison report after the core final synthesis. It supports Claude Code, Codex, Pi, and custom command-line adapters. This package installs the same orchestrator on Linux and macOS; it does not bundle or purchase agent CLIs or models. This release is version `1.15.0`.
...
```bash
shasum -a 256 -c review-pr-1.15.0.tar.gz.sha256  # macOS
# or: sha256sum -c review-pr-1.15.0.tar.gz.sha256 # Linux
tar -xzf review-pr-1.15.0.tar.gz
cd review-pr-1.15.0
