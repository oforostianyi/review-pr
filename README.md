...
`reporting.finding_contract.primary` and `cross_review` accept `markdown` (the backward-compatible default) or opt-in `ndjson-v1`. Structured cross-review requires structured primary findings and `comparison_sections.cross_review: "none"`. Primary agents return atomic NDJSON records; cross-reviewers classify every supplied source finding and identify it with an unambiguous `{agent, source_id}` reference. The orchestrator validates completeness, provenance, severity, and exact changed-line anchors, preserves `work/*-raw.ndjson`, publishes canonical `work/*-findings.json`, and renders localized Markdown for humans and final synthesis. A structurally repairable primary or cross-review response receives one content-stable schema-repair pass before an ordinary whole-review retry. Cross-review repair additionally freezes classification, severity, provenance, and contributing agents. Invalid structured output never falls back to Markdown. Existing manifests without the settings remain Markdown runs, while resume uses the modes saved in the manifest. Final synthesis remains Markdown in this staged rollout. The installed `review-pr-findings-v1.schema.json` documents the portable record shape.
